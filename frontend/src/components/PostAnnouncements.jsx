          ...
          <ul className="mt-4 space-y-3">
            {courses.map((course) => (
              <li
                ...
                className="p-3 flex items-center gap-4 bg-cl3 text-white rounded-lg shadow cursor-pointer hover:bg-cl4 transition"
                onClick={() => setSelectedCourse(course.courseCode)}
              >
                <div className="w-10 h-10 flex items-center justify-center bg-cl4 text-white rounded-full font-bold">
                  {course.courseCode.slice(0, 1)}
                </div>
                <div>
                  <p className="text-lg font-medium">{course.courseName}</p>
                  <p className="text-sm text-cl5">{course.courseParentName}</p>
                </div>
              </li>
            ))}
          </ul>
